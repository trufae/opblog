...
function createMarkdownIndex(results) {
  const rows = [];
  results.forEach(r => {
    rows.push(`- [${r.name}](${r.name}.html)`);
  });
  return rows.join('\n');
}

function preProcess(templateFile, post, results) {
  const outFile = post.htmlPath;
  ...
  */
    ...
    if (tok) {
      ...
      if (tok.name[0] == '$') {
        ...
        switch (tok.name) {
          case 'post':
            if (tok.body === 'index') {
              newdata += parseMarkdown (createMarkdownIndex(results));
            } else {
              newdata += parseMarkdown (post[tok.body]);
            }
            break;
          case 'error':
          case 'comment':
  ...
  if (!outFile) {
    outFile = file.replace (/\.(md|tmpl)/, ".html");
  }
  if (file == outFile) {
...
if (process.argv.length > 2) {
  process.argv.forEach(preProcess);
}
